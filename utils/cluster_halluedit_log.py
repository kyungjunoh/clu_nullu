...
class HalluEditCluster():
    ...
    def svd_on_edit_features(self, edit_features):
        # SVD 수행 (기존과 동일하지만, 평균 벡터 반환 기능 추가)
        svd = {}
        cluster_means = {} # ★ 추가: 클러스터 평균 벡터 저장용

        for key in edit_features:
            M = edit_features[key].to(torch.float32)
            
            # ★ 클러스터의 '대표 성향'(평균)을 저장해둡니다. (Before/After 비교용)
    ...
    def find_p_hallu(self, svd, cluster_means): # ★ 인자 추가: cluster_means
        ...
        # 헤더 작성
        self.f.write("\n" + "="*60 + "\n")
        self.f.write("   HalluSpace Analysis: Before vs After Projection\n")
        self.f.write("="*60 + "\n")
        ...
        for key in svd.keys():
            ...
            mean_vec = cluster_means[key].to('cuda') # 클러스터 평균 벡터
            ...
            # --- [2] After: 편집 후(투영 후) 남는 단어 시뮬레이션 ---
            # mean_vec_new = (I - P) @ mean_vec
            # P는 cuda/cpu 혼용 주의
        ...
        for layer_num, cluster_dict in clustered_diff_matrices_by_layer.items():
            for cluster_id, features in cluster_dict.items():
                ...
                if target_key:
                    ...
                    # ★ 수정됨: svd 함수가 cluster_means도 반환함
